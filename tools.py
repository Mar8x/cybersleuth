...
def dns_twist_impl(domain: str, extra_permutations=None, fuzzers=None, resolve: bool = True,
                   whois: bool = False, limit: int = 150) -> dict:
    """DNS-twist: generate domain permutations (typosquat/homoglyph/bitsquat/tld/subdomain) via dnstwist,
    resolve them, and also check any AI-supplied `extra_permutations`. Returns only positive
    (registered/resolving) hits, capped at `limit`. Synchronous (auto-threaded by the server)."""
    started = datetime.datetime.now(_UTC)
    ...
    domain = domain.split("/")[0]
    if not domain:
        return {"error": "empty domain", "_telemetry": _make_telemetry(started, errors=["empty domain"], quality=0.0)}
    ...
    try:
        import dnstwist
        kwargs = {"domain": domain, "format": "null", "registered": bool(resolve)}
        if fuzzers:
            kwargs["fuzzers"] = fuzzers if isinstance(fuzzers, str) else ",".join(fuzzers)
        if whois:
            kwargs["whois"] = True
        rows = dnstwist.run(**kwargs) or []
        checked += len(rows)
        for r in rows:
            v = (r.get("domain") or "").lower()
            if not v or v == domain or v in seen:
                continue
            seen.add(v)
            registered = bool(r.get("dns_a") or r.get("dns_ns") or r.get("dns_mx"))
            if resolve and not registered:
                continue
            hits.append({
                "variant": v,
                "fuzzer": r.get("fuzzer"),
                "dns_a": r.get("dns_a") or [],
                "dns_aaaa": r.get("dns_aaaa") or [],
                "dns_mx": r.get("dns_mx") or [],
                "dns_ns": r.get("dns_ns") or [],
                "registrar": r.get("whois_registrar"),
                "whois_created": r.get("whois_created"),
                "geoip": r.get("geoip"),
                "registered": True,
            })
    except Exception as e:
        return {"error": f"dnstwist failed: {e}", "domain": domain,
                "_telemetry": _make_telemetry(started, errors=[str(e)], quality=0.0)}

    # Resolve any AI-supplied extra permutations ourselves (not generated by the fuzzers).
    extras = [p.strip().lower() for p in (extra_permutations or []) if p and p.strip()]
    if extras:
        resolver = dns.resolver.Resolver()
        resolver.timeout = 5
        resolver.lifetime = 5
        for perm in extras:
            checked += 1
            if perm == domain or perm in seen:
                continue
            try:
                a = [str(x) for x in resolver.resolve(perm, "A")]
            except Exception:
                a = []
            if a:
                seen.add(perm)
                hits.append({"variant": perm, "fuzzer": "ai-pattern", "dns_a": a, "dns_aaaa": [],
                             "dns_mx": [], "dns_ns": [], "registrar": None, "whois_created": None,
                             "geoip": None, "registered": True})

    hits = hits[: max(1, int(limit or 150))]
    return {
        "domain": domain,
        "hits": hits,
        "hit_count": len(hits),
        "checked": checked,
        "_telemetry": _make_telemetry(started, quality=1.0 if hits else 0.5),
    }
